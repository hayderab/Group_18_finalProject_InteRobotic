...
def create_dict_grid(dict_grid, x_index, y_index, x, y, occ_grid_value):
    if occ_grid_value > 0:  # for obstacles
        dict_grid[x_index, y_index] = [x, y, -100.0, 0.0, (-1, -1), False, "DEFAULT"]  # reward = -1
    elif occ_grid_value == -1:  # for unknown cells
        dict_grid[x_index, y_index] = [x, y, -100.0, 0.0, (-1, -1), False,
        ...
        if (8 <= x <= 10) and (9 <= y <= 11):  # for the target goal cell (I chose for now the centre of the maze)
            dict_grid[x_index, y_index] = [x, y, 10.0, 0.0, (-1, -1), True, "DEFAULT"]  # reward = 10
        else:
...
def maze_dict_grid(data):
    ...
    for y_index in range(0, data.info.height, 2):
        for x_index in range(0, data.info.width, 2):
            ...
            maze_dict = create_dict_grid(maze_dict, x_index, y_index, x, y, occ_grid_value)
